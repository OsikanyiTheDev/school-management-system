...
from shared.validation import validate_student_profile
...
def lambda_handler(event, context):  # noqa: ARG001
    ...
    clean, errors = validate_student_profile(payload)
    if errors:
        return http.bad_request(errors)
    item = build_person(clean, person_type="student", id_prefix="stu", created_by=authz.caller_sub(event) or "unknown")
    try:
        repo.put_item(item)
    except RepositoryConflict:
        return http.bad_request(["student already exists"])
    return http.created({"student": public_item(item)})
