...
from shared.domain import build_term
from shared.repository import RepositoryConflict, SmisRepository
from shared.validation import validate_term
...
def lambda_handler(event, context):  # noqa: ARG001
    ...
    clean, errors = validate_term(payload)
    if errors:
        return http.bad_request(errors)
    item = build_term(clean, created_by=authz.caller_sub(event) or "unknown")
    try:
        repo.put_item(item)
    except RepositoryConflict:
        return http.bad_request(["term already exists"])
    return http.created({"term": public_item(item)})
