"""POST /schools/{school_id}/teacher-assignments."""
...
from shared.domain import build_teacher_assignment
from shared.repository import RepositoryConflict, SmisRepository
from shared.validation import validate_teacher_assignment
...
def lambda_handler(event, context):  # noqa: ARG001
    ...
    clean, errors = validate_teacher_assignment(payload)
    if errors:
        return http.bad_request(errors)
    item = build_teacher_assignment(clean, created_by=authz.caller_sub(event) or "unknown")
    try:
        repo.put_item(item)
    except RepositoryConflict:
        return http.bad_request(["teacher assignment already exists"])
    return http.created({"teacher_assignment": public_item(item)})
