...
from shared.domain import build_academic_year
from shared.repository import RepositoryConflict, SmisRepository
from shared.validation import validate_academic_year
...
def lambda_handler(event, context):  # noqa: ARG001
    ...
    clean, errors = validate_academic_year(payload)
    if errors:
        return http.bad_request(errors)
    item = build_academic_year(clean, created_by=authz.caller_sub(event) or "unknown")
    try:
        repo.put_item(item)
    except RepositoryConflict:
        return http.bad_request(["academic year already exists"])
    return http.created({"academic_year": public_item(item)})
