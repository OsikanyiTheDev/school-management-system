...
from shared.validation import validate_teacher_profile
...
def lambda_handler(event, context):  # noqa: ARG001
    ...
    clean, errors = validate_teacher_profile(payload)
    if errors:
        return http.bad_request(errors)
    item = build_person(clean, person_type="teacher", id_prefix="tch", created_by=authz.caller_sub(event) or "unknown")
    try:
        repo.put_item(item)
    except RepositoryConflict:
        return http.bad_request(["teacher already exists"])
    return http.created({"teacher": public_item(item)})
