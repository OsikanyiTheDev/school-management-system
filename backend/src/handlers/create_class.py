...
from shared.domain import build_class
from shared.repository import RepositoryConflict, SmisRepository
from shared.validation import validate_class
...
def lambda_handler(event, context):  # noqa: ARG001
    ...
    clean, errors = validate_class(payload)
    if errors:
        return http.bad_request(errors)
    item = build_class(clean, created_by=authz.caller_sub(event) or "unknown")
    try:
        repo.put_item(item)
    except RepositoryConflict:
        return http.bad_request(["class already exists"])
    return http.created({"class": public_item(item)})
